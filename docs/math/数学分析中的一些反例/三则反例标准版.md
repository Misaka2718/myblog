---
title: 三则反例标准版
createTime: 2026/01/15 20:29:40
permalink: /math/by7a179u/
---
...
整数点上和f\left( n\right)一致:{f}_{n}\left( n\right)  = f\left( n\right) ,\;{f}_{n}\left( {n + 1}\right)  = f\left( {n + 1}\right) ,
$$

$$
中点取两端平均值:{f}_{n}\left( {n + 1/2}\right)  = \{ f\left( n\right)  + f\left( {n + 1}\right) \} /2,
$$

$$
两端平滑中间陡峭:{f}_{n}^{\prime }\left( n\right)  = {f}_{n}^{\prime }\left( {n + 1}\right)  = 0,\;{f}_{n}^{\prime }\left( {n + 1/2}\right)  = 1.
$$
...
![原点附近](origin.jpg)
$$
图1.\quad x + 2x^2 \sin \dfrac{1}{x}
$$
![导数](Dfx.jpg)
$$
图2.\quad 1 + 4x \sin \dfrac{1}{x} - 2 \cos \dfrac{1}{x}
$$
...
$$
图4.\quad 无限可微平滑过渡函数（Smooth Transition Function）
$$
###  基于 \(e^{-1/x^2}\) 的积分构造
\[
\phi(x) =
